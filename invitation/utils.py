...
def get_registration_backend_class(backend_str=None):
    return str_to_class(backend_str, 'INVITATION_BACKEND', 'invitation.backends.AllAuthRegistrationBackend')

def get_delivery_backend_class(delivery_str=None):
    return str_to_class(delivery_str, 'INVITATION_DELIVERY_BACKEND', 'invitation.backends.EmailDeliveryBackend')

def get_invitation_form(form_str=None):
    return str_to_class(form_str, 'INVITATION_FORM', 'invitation.forms.DefaultInvitationKeyForm')
    
def get_token_generator_class(generator_str=None):
    return str_to_class(generator_str, 'INVITATION_TOKEN_GENERATOR', 'invitation.utils.DefaultTokenGenerator')

def str_to_class(class_str, settings_key="", default_str=""):
    class_str = class_str or getattr(settings, settings_key, default_str)
    mod, sep, kls = class_str.rpartition('.')
...
class DefaultTokenGenerator(BaseTokenGenerator):
    def generate_token(self, instance, invitation_url):
        ...
        if not default_storage.exists('tokens/%s.png' % instance.key):
            default_storage.save('tokens/%s.png' % instance.key, File(temp_img))
        get_token_url = root_url+reverse('invitation_token', kwargs={'key':instance.key})
